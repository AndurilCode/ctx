...
async function importHarnessModule() {
  const candidates = [
  ...
  throw new Error('Harness module unavailable');
}
...
export async function evaluateHarness({ event, toolName, toolInput, rawPath, prompt }) {
  ...
  if (event === 'UserPromptSubmit' && prompt) {
    try {
      ...
      const { buildProfile, createHarnessState, serialize, deserialize, acquireLock, releaseLock } = harness;
      const statePath = `${process.cwd()}/.claude/harness-state.json`;
  ...
  if (event === 'Stop') {
    try {
      const harness = await importHarnessModule();
      const { deserialize, computeMetrics, acquireLock, releaseLock } = harness;
      const statePath = `${process.cwd()}/.claude/harness-state.json`;
      const lockPath = `${statePath}.lock`;
      const locked = acquireLock(lockPath, 500);
      if (locked) {
  ...
  try {
    const harness = await importHarnessModule();
    const { createHarnessState, decide, serialize, deserialize, recordToolCall, acquireLock, releaseLock } = harness;
